  ...
  useEffect(() => {
    ...
    const fetchFacility = async () => {
      try {
        ...
        const response = await fetch(`https://deporsm-apiwith-1035693188565.us-central1.run.app/api/instalaciones/${parsedId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        const response = await fetch('https://deporsm-apiwith-1035693188565.us-central1.run.app/api/reservas', {
          method: 'POST',
          headers: {
        ...
        if (paymentMethod === 'online') {
          if (formData.cardNumber.endsWith('1234')) { // Simular fallo para tarjetas específicas
            ...
            try {
              const updateResponse = await fetch(`https://deporsm-apiwith-1035693188565.us-central1.run.app/api/reservas/${reservationId}/actualizar-pago`, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                },
          ...
          try {
            ...
            const pagoResponse = await fetch('https://deporsm-apiwith-1035693188565.us-central1.run.app/api/pagos/deposito', {
              method: 'POST',
              credentials: 'include',
              body: formData
            });
